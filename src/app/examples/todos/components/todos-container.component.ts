...
export class TodosContainerComponent implements OnInit, OnDestroy {
  ...
  onAddTodo() {
    ...
    const addedMessage = this.translateService.instant(
      'lottonini.examples.todos.added.notification',
      { name: this.newTodo }
    );
  ...
  onToggleTodo(todo: Todo) {
    ...
    const newStatus = this.translateService.instant(
      `lottonini.examples.todos.filter.${todo.done ? 'active' : 'done'}`
    );
    const undo = this.translateService.instant('lottonini.examples.todos.undo');
    const toggledMessage = this.translateService.instant(
      'lottonini.examples.todos.toggle.notification',
      { name: todo.name }
    );
  ...
  onRemoveDoneTodos() {
    ...
    const removedMessage = this.translateService.instant(
      'lottonini.examples.todos.remove.notification'
    );
  ...
  onFilterTodos(filter: TodosFilter) {
    ...
    const filterToMessage = this.translateService.instant(
      'lottonini.examples.todos.filter.notification'
    );
    const filterMessage = this.translateService.instant(
      `lottonini.examples.todos.filter.${filter.toLowerCase()}`
    );
